      ...
      try {
        ...
        const topUpPositionCall = this.provider.eth.abi.encodeFunctionCall(
          ...
          [
            ...
            SmartContractFactory.StablecoinAdapter(this.chainId).address,
            positionId,
            fathomToken ? this.provider.utils.toWei(fathomToken.toString(), "ether") : "0",
            encodedResult
          ]
        );
        ...
        const options = {
          from: address,
          gas: 0,
          value: collateral ? this.provider.utils.toWei(collateral, "ether") : 0
        };
        const gas = await getEstimateGas(
          wallet,
          "execute",
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const options = {
          from: address,
          gas: 0,
          value: collateral ? this.provider.utils.toWei(collateral.toString(), "ether") : "0"
        };
  ...
  closePosition(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const wipeAllAndUnlockTokenCall = this.provider.eth.abi.encodeFunctionCall(
          jsonInterface,
          [
            SmartContractFactory.PositionManager(this.chainId).address,
            pool.tokenAdapterAddress,
